 ...
 * @param {string} folder   - Cloudinary folder (e.g. 'eeoxoe/images')
 * @returns {Promise<object>} Cloudinary upload result (contains .secure_url, .public_id etc.)
 */
const uploadToCloudinary = (buffer, folder = 'eeoxoe/images') => {
 ...
 */
const handleImageUpload = (folder = 'eeoxoe/images') => [
  upload.single('image'),
  async (req, res, next) => {
